...
async function orchestrate(config) {
  ...
  try {
    ...
    aiOrchestrator.setCachedSiteContext(discovery);
    let aiCost = 'N/A';
    ...
    try { gitSha = require('child_process').execSync('git rev-parse --short HEAD', { encoding: 'utf-8' }).trim(); } catch (e) {
      log(ctx, '⚠️', `Git SHA unavailable: ${e.message}`);
    }
    ...
    const notifyPromises = [
      sendSlackNotification(notificationData).catch(e => log(ctx, '⚠️', `Slack notification failed: ${e.message}`)),
      sendEmailNotification(notificationData).catch(e => log(ctx, '⚠️', `Email notification failed: ${e.message}`)),
    ];

    // File bugs to Jira/Linear
    if (ctx.bugs.length > 0) {
      notifyPromises.push(fileJiraBugs(ctx.bugs, config.siteName).catch(e => { log(ctx, '⚠️', `Jira filing failed: ${e.message}`); return []; }));
      notifyPromises.push(fileLinearBugs(ctx.bugs, config.siteName).catch(e => { log(ctx, '⚠️', `Linear filing failed: ${e.message}`); return []; }));
    }
