...
const { deployToGitPages } = require('../reporting/deployer');
const { sendSlackPDF } = require('../notifications/slack');
...
async function orchestrateComparison(uatUrl, prodUrl, config) {
  ...
  try {
    ...
    const totalDiffs = s.diffSteps || 0;
    await deployToGitPages({
      reportPath,
      siteName: compCtx.uatUrl.replace(/https?:\/\//, '').replace(/[/.]/g, '-').replace(/-+$/, ''),
      bugCount: totalDiffs,
      bugCounts: {
        critical: s.functionalDiffs || 0,
        high: s.visualDiffs || 0,
        medium: s.structuralDiffs || 0,
        low: s.dataDiffs || 0,
      },
      type: 'comparison',
      compareSite: compCtx.prodUrl.replace(/https?:\/\//, '').replace(/[/.]/g, '-').replace(/-+$/, ''),
    });

    // Send report PDF to Slack
    await sendSlackPDF(reportPath, {
      siteName: `${compCtx.uatUrl} vs ${compCtx.prodUrl}`,
      totalBugs: totalDiffs,
      critical: s.functionalDiffs || 0,
      high: s.visualDiffs || 0,
      medium: s.structuralDiffs || 0,
      low: s.dataDiffs || 0,
