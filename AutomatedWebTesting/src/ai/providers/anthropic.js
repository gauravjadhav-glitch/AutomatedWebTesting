...
class AnthropicProvider {
  ...
  /**
   * Set a system prompt to be cached across multiple calls.
   * Reduces token usage when the same context (site structure, discovery) is reused.
   */
  setCachedContext(systemPrompt) {
   ...
   * Uses prompt caching for system messages to reduce costs.
   */
  async chat(model, messages, maxTokens = 500) {
    try {
      ...
      const systemText = this.cachedSystemPrompt
        ? [this.cachedSystemPrompt, ...systemMsgs.map(m => m.content)].join('\n\n')
