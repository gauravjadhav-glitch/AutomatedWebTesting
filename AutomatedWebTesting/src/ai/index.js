...
class AIOrchestrator {
  ...
  /**
   * Cache site context for the Anthropic provider to reuse across bug analysis calls.
   * This reduces token costs by ~70% when analyzing multiple bugs from the same site.
   */
  setCachedSiteContext(discovery) {
    if (this.provider && typeof this.provider.setCachedContext === 'function') {
      const context = `You are a QA bug analyst for "${discovery.siteName}" (${discovery.baseUrl}). Platform: ${discovery.platform}. Live pages: ${discovery.livePages.length}. Features: ${Object.entries(discovery.features).filter(([,v]) => v).map(([k]) => k).join(', ')}.`;
      this.provider.setCachedContext(context);
    }
  }
